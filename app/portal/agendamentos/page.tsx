...
// Componentes da UI e Tipos
import { Service, Booking } from "@/types";
import { BookingsList } from '@/components/portal/BookingsList'; // <-- NOSSO COMPONENTE REUTILIZÁVEL
...
import { Loader2, CalendarPlus, Info, Send, Calendar as CalendarIcon } from 'lucide-react';
...
export default function GuestServicesPage() {
    ...
    useEffect(() => {
        ...
        const allBookingsQuery = firestore.query(
            firestore.collection(db, 'bookings'), 
            firestore.where('date', '==', dateStr), 
            firestore.where('serviceId', '==', selectedService.id)
        );
    ...
    return (
        <div className="space-y-8">
            ...
            <header>
                <h1 className="text-3xl font-bold">Agendamento de Serviços</h1>
                <p className="text-lg text-muted-foreground">Explore e solicite as experiências que oferecemos.</p>
            </header>

            {/* ## SEÇÃO "MEUS AGENDAMENTOS" REATORADA ## */}
            {/* Usando o componente BookingsList para exibir os agendamentos existentes */}
            <BookingsList
                bookings={bookings}
                title="Meus Agendamentos"
                description="Acompanhe o status de todas as suas solicitações."
                showPast={true} // Mostra o histórico completo nesta página
            />
                        ...
                        <Form {...form}>
                            <form id="booking-form" onSubmit={form.handleSubmit(handleBookingSubmit)} className="space-y-4 py-4">
                                ...
                                {selectedService.type === 'on_demand' ? (
                                    ...
                                    <div className="flex items-center justify-center text-center p-3 bg-secondary rounded-md">
                                        <CalendarIcon className="mr-2 h-5 w-5 text-secondary-foreground"/>
