        ...
        {loading ? <div className="flex justify-center p-4"><Loader2 className="animate-spin" /></div> :
         activeRequests.length > 0 ? (
          <div className="space-y-4">
            ...
            <Button variant="outline" className="w-full asChild"><Link href="/solicitar?tab=tracking">Ver todos</Link></Button>
          </div>
        ) : (
          <div className="text-center text-sm text-muted-foreground py-4">
            <p className="mb-4">Nenhuma solicitação ativa no momento.</p>
            <Button className="w-full asChild"><Link href="/solicitar">Fazer uma solicitação</Link></Button>
          </div>
        )}
      </CardContent>
    </Card>
